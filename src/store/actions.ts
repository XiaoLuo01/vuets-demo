...
const actions: ActionTree<any,any> = {
  async setUser({state, commit}, user: any) {
    ...
    const { key } = decoded;
    // 返回当前用户拥有权限的路由
    let accessedRouters = fileterAsyncRouter(asyncRouterMap, key);
 ...
 * @param roles 当前角色
 */
function fileterAsyncRouter(asyncRouterMap: Array<any>, roles: string) {
...
function hasPermission(roles: string, route: any) {
  if (route.meta && route.meta.roles) {
    // meta.roles 是否包含角色的 key 值,如果包含那么就是有权限,否则无权限
